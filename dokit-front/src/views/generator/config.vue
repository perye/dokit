      ...
      el-col(style="margin-bottom: 10px")
        el-card.box-card(shadow="never")
          .clearfix(slot="header")
            ...
            el-tooltip.item(effect="dark" content="数据库中表字段变动时使用该功能" placement="top-start")
              el-button(
                :loading="syncLoading"
            ...
            el-table(v-loading="loading" :data="data" :max-height="tableHeight" size="small" style="width: 100%;margin-bottom: 15px")
              ...
              el-table-column(prop="remark" label="字段描述")
                template(slot-scope="scope")
                  el-input.edit-input(v-model="data[scope.$index].remark" size="mini")/
              ...
              el-table-column(label="表单类型")
                template(slot-scope="scope")
                  el-select.edit-input(v-model="data[scope.$index].formType" filterable clearable size="mini" placeholder="请选择")
              ...
              el-table-column(label="查询方式")
                template(slot-scope="scope")
                  el-select.edit-input(v-model="data[scope.$index].queryType" filterable clearable size="mini" placeholder="请选择")
              ...
              el-table-column(label="日期注解")
                template(slot-scope="scope")
                  el-select.edit-input(v-model="data[scope.$index].dateAnnotation" filterable clearable size="mini" placeholder="请选择")
                    el-option(label="自动创建时间" value="CreationTimestamp")/
                    el-option(label="自动更新时间" value="UpdateTimestamp")/
              el-table-column(label="关联字典")
                template(slot-scope="scope")
                  el-select.edit-input(v-model="data[scope.$index].dictName" filterable clearable size="mini" placeholder="请选择")
        ...
        el-card.box-card(shadow="never")
          ...
          el-form(ref="form" :model="form" :rules="rules" size="small" label-width="78px")
            el-form-item(label="作者名称" prop="author")
              ...
              span(style="color: #C0C0C0;margin-left: 10px;") 类上面的作者名称
            el-form-item(label="至于包下" prop="pack")
              el-input(v-model="form.pack" style="width: 40%")/
              span(style="color: #C0C0C0;margin-left: 10px;") 项目包的名称，生成的代码放到哪个包里面
            el-form-item(label="接口名称" prop="apiAlias")
              el-input(v-model="form.apiAlias" style="width: 40%")/
              span(style="color: #C0C0C0;margin-left: 10px;") 接口的名称，用于控制器与接口文档中
            el-form-item(label="前端路径" prop="path")
              el-input(v-model="form.path" style="width: 40%")/
              span(style="color: #C0C0C0;margin-left: 10px;") 输入views文件夹下的目录，不存在即创建
            el-form-item(label="去表前缀" prop="prefix")
              el-input(v-model="form.prefix" placeholder="默认不去除表前缀" style="width: 40%")
              span(style="color: #C0C0C0;margin-left: 10px;") 默认不去除表前缀，可自定义
            el-form-item(label="是否覆盖" prop="cover")
              el-radio-group(v-model="form.cover" size="mini" style="width: 40%")
                el-radio-button(label="true") 是
                el-radio-button(label="false") 否
              span(style="color: #C0C0C0;margin-left: 10px;") 谨防误操作，请慎重选择
</template>
